...
/**
 * Service interface for managing scan records of products in the conveyor belt
 * scanning system.
 * <p>
...
public interface ScanService {
     ...
     *
     * @param productId    the UUID of the product that was scanned; must not be
     *                     null
     * @param isSuccess    flag indicating whether the scan was successful; must not
     *                     be null
     * @param errorMessage an optional error message; should be null or empty if
     *                     {@code isSuccess} is true
     ...
     *
     * @param filter a {@link ScanFilter} object containing search conditions such
     *               as date ranges,
     *               product IDs, or success status; must not be null
     * @return a list of scan records that match the filter; never null, may be
     *         empty
     */
    List<Scan> find(ScanFilter filter);

    /**
     * Generates general statistics for all scan records within the specified time
     * range.
     *
     * @param startDate the beginning of the time range (inclusive); must not be
     *                  null
     * @param endDate   the end of the time range (inclusive); must not be null and
     *                  not before {@code startDate}
     * @return a {@link GeneralStatistics} object containing aggregated scan data
     *         (e.g., total scans, success/failure rate)
     * @throws IllegalArgumentException if the date range is invalid or any
     *                                  parameter is null
     */
     ...
     *
     * @param productId the UUID of the product to generate statistics for; must not
     *                  be null
     * @param startDate the beginning of the time range (inclusive); must not be
     *                  null
     * @param endDate   the end of the time range (inclusive); must not be null and
     *                  not before {@code startDate}
     * @return a {@link ProductStatistics} object with metrics like scan count,
     *         error rate, and last scan timestamp
